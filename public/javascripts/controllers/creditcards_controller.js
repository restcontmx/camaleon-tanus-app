yukonApp
.factory( 'CreditCardRepository',  [ '$http',  function( $http ) {
    return({
    ...
    if( AuthRepository.viewVerification() ) {
        ...
        $scope.locations_select_change = function() {
            ...
            if ( locations.length == 0 || locations.length == $scope.locations.length ) {
                $scope.gridOptions.data = $scope.crerditcard_reports;
            } else {
                $scope.gridOptions.data = $scope.crerditcard_reports.filter( c => ( locations.find( l => ( c.location == l.id ) ) ) );
            }
        };
    }
}]);