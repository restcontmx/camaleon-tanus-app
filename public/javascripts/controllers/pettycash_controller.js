yukonApp
    .factory('PettyCashRepository', ['$http', function ($http) {
        return ({
            reportsByDate: (d1, d2, turn_id) => $http.get('/reports/pettycash/?d1=' + d1 + '&d2=' + d2 + '&turn=' + turn_id)
        });
    }])
    .controller('pettycash-reports-controller', [   '$scope',
                                                    '$rootScope',
                                                    'AuthRepository',
                                                    'PettyCashRepository',
                                                    'TurnRepository',
            ...
            if (AuthRepository.viewVerification()) {
                ...
                $scope.get_reports = function () {
                    ...
                    PettyCashRepository.reportsByDate(date_1, date_2, turn_id).success(function (data) {
                        if (!data.error) {
                            $scope.pettycash_reports = data.data.petty_cash_reports;
                            $scope.gridOptions.data = $scope.pettycash_reports
                            // Get locations
                ...
                $scope.locations_select_change = function () {
                    ...
                    if (locations.length == 0 || locations.length == $scope.locations.length) {
                        ...
                        $scope.gridOptions.data = $scope.pettycash_reports.filter(c => (locations.find(l => (c.location == l.id))));
                    }
                };
            }
        }]);