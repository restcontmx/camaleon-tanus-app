yukonApp
    .factory('EmployeeRepository', ['$http', function ($http) {
        return ({
            reportsByDate: (d1, d2 ) => $http.get('/reports/it_temployee/?d1=' + d1 + '&d2=' + d2 )
        });
    }])
    .controller('employee-reports-controller', [    '$scope', 
                                                    '$rootScope', 
                                                    'AuthRepository',
                                                    'EmployeeRepository',
                                                    'TurnRepository',
        ...
        if( AuthRepository.viewVerification() ) {
            ...
            $scope.get_reports = function() {
                ...
                EmployeeRepository.reportsByDate( date_1, date_2 ).success( function( data ) {
                    if( !data.error ) {
                        $scope.reports = data.data.reports;
            ...
            $scope.locations_select_change = function() {
                ...
                if ( locations.length == 0 || locations.length == $scope.locations.length ) {
                    ...
                    $scope.gridOptions.data = $scope.reports.filter( c => ( locations.find( l => ( c.location == l.id ) ) ) );
                }
            };
        }
    }]);
