yukonApp
    .factory( 'DepartmentRepository', [ '$http', function( $http ) {
        var model = 'it_tdepartment';
        return({
            reportsByDate : ( d1, d2, turn_id ) => $http.get( '/reports/' + model + '/?d1=' + d1 + '&d2=' + d2 + '&turn=' + turn_id )
        });
    }])
    .controller( 'department-controller', [ '$scope',
                                            'LocationRepository',
                                            'AuthRepository',
                                            function(   $scope,
                                                        LocationRepository,
    ...
    .controller('department-reports-controller', [  '$scope',
                                                    'LocationRepository',
        ...
        if( AuthRepository.viewVerification() ) {
            ...
            $scope.get_reports = function() {
                ...
                DepartmentRepository.reportsByDate( date_1, date_2, turn_id ).success( function( data ) {
                    if( !data.error ) {
                        $scope.department_reports = data.data.department_reports;
                        $scope.department_reports_all = data.data.department_reports_all;
                        $scope.department_reports_all_table = data.data.department_reports_all;
                        $scope.top10_donut_data = [];
                        $scope.global_total = $scope.department_reports_all_table.map( r => parseFloat( r.total ) ).reduce( ( a, b ) => ( a + b ), 0 );
                        $scope.department_reports_all_table.slice(0, 10).forEach( r => $scope.top10_donut_data.push( [ r.depa_name, r.total ] ) );
            ...
            $scope.locations_select_change = function() {
                ...
                if ( locations.length == 0 || locations.length == $scope.locations.length ) {
                    $scope.department_reports_all_table = $scope.department_reports_all;
                    $scope.global_total = $scope.department_reports_all_table.map(r => parseFloat(r.total)).reduce((a, b) => (a + b), 0);
                } else {
                    $scope.department_reports_all_table = $scope.department_reports.filter( c => ( locations.find( l => ( c.location == l.id ) ) ) );
                    $scope.global_total = $scope.department_reports_all_table.map(r => parseFloat(r.total)).reduce((a, b) => (a + b), 0);
                }
                $scope.top10_donut_data = [];
                $scope.department_reports_all_table.slice(0, 10).forEach( r => $scope.top10_donut_data.push( [ r.depa_name, r.total ] ) );
            ...
            var set_compare_table = function() {
                ...
                $scope.locations.forEach( l => {
                    let reports = $scope.department_reports.filter( r => r.location == l.id );
